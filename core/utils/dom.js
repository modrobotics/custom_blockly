...
Blockly.utils.dom.createSvgElement = function(name, attrs, parent) {
  const ledPath = 'm 16,0  h 8 a 16 16 0 0 1 0,32  V 32  h -8 a 16 16 0 0 1 0,-32 z'
  let transformValue = null;

  // Iterate through all g elements to find the one with the target d attribute value
  const svgElements = document.querySelectorAll('svg g');
  svgElements.forEach(gElement => {    
    var serializer = new XMLSerializer();
    var svgString = '<svg xmlns="http://www.w3.org/2000/svg">' + serializer.serializeToString(gElement) + '</svg>';;

    if (svgString.includes(ledPath)) {

      let tr = gElement.querySelector('path');
      if (tr && tr?.getAttribute('d')) {
        transformValue = gElement.getAttribute('transform')
        switch (transformValue) {
          case "translate(48,8)":
            gElement.setAttribute('transform', "translate(75,11)");
            break;
          case "translate(8,48)":
            gElement.setAttribute('transform', "translate(119,28)");
            break;
          case "translate(8,88)":
            gElement.setAttribute('transform', "translate(133,69)");
            break;
          case "translate(8,128)":
            gElement.setAttribute('transform', "translate(119,109)");
            break;
          case "translate(8,168)":
            gElement.setAttribute('transform', "translate(75,125)");
            break;
          case "translate(8,208)":
            gElement.setAttribute('transform', "translate(29,109)");
            break;
          case "translate(8,248)":
            gElement.setAttribute('transform', "translate(12,69)");
            break;
          case "translate(8,288)":
            gElement.setAttribute('transform', "translate(29,28)");
            break;
          default:
            break;
        }
